...
// Speech Recorder Class - Audio recording only
export class SpeechRecognizer {
  constructor() {
    ...
    this.active = false
    this.onError = null
    // We keep onResult to mimic the interface but it won't return partial transcripts anymore
    this.onResult = null
  }
  ...
  async start() {
    ...
    try {
      ...
      this.mediaRecorder.start(200) // Collect chunks frequently
    } catch (error) {
      console.error('Microphone access error:', error)
      this.active = false
      if (this.onError) {
        this.onError(new Error('Microphone permission denied or not available. ' + error.message))
      }
    }
  }
  ...
  async stop() {
    ...
    return {
      text: null, // No transcription
      audioBlob
    }
  }
}

// Unused - removed completeness/accuracy check
